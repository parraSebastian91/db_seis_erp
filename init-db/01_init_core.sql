...
CREATE TABLE core.grupo_trabajo (
    ...
    descripcion TEXT,
    lider_usuario_uuid UUID REFERENCES core.usuario(usuario_uuid), -- Jefe del grupo
...
CREATE TABLE core.grupo_miembro (
    ...
    grupo_id UUID NOT NULL REFERENCES core.grupo_trabajo(grupo_id) ON DELETE CASCADE,
    ...
    jefe_directo_id UUID REFERENCES core.grupo_miembro(miembro_id), 
    cargo_en_grupo VARCHAR(100),
    grupo_metadata JSONB DEFAULT '{}'::jsonb,
    fecha_ingreso TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now(),
    active BOOLEAN DEFAULT true,
    UNIQUE(grupo_id, usuario_uuid) -- Un usuario solo puede ser miembro una vez por grupo
);
...
ALTER TABLE core.permiso ADD permiso_tipo core."permiso_tipo" NULL;
...
BEGIN
    -- Caso raíz: si se intenta auto-referenciar, se normaliza a NULL.
    -- Esto permite el caso de "miembro único" sin romper la FK recursiva.
    IF NEW.jefe_directo_id = NEW.miembro_id THEN
        NEW.jefe_directo_id := NULL;
    END IF;
