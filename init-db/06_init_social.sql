...
CREATE TABLE social.publicacion (
    ...
    post_uuid UUID NOT NULL DEFAULT gen_random_uuid() UNIQUE,
...
CREATE TABLE social.post_attachments (
    post_id UUID REFERENCES social.publicacion(post_uuid),
...
CREATE TABLE social.calificacion_trabajo (
    ...
    post_id BIGINT NOT NULL REFERENCES social.publicacion(post_id) ON DELETE CASCADE,
...
CREATE TABLE social.reaccion_post (
    post_id BIGINT REFERENCES social.publicacion(post_id) ON DELETE CASCADE,
...
CREATE TABLE social.mencion_post (
    ...
    post_id BIGINT REFERENCES social.publicacion(post_id),
    organizacion_mencionada_id BIGINT REFERENCES core.organizacion(organizacion_id),
    created_at TIMESTAMPTZ DEFAULT now()
);

ALTER TABLE social.publicacion 
ADD COLUMN estado_multimedia VARCHAR(20) DEFAULT 'PENDING'; 
...
BEGIN
    IF (TG_OP = 'INSERT') THEN
        UPDATE social.publicacion SET count_likes = count_likes + 1 WHERE post_id = NEW.post_id;
    ELSIF (TG_OP = 'DELETE') THEN
        UPDATE social.publicacion SET count_likes = count_likes - 1 WHERE post_id = OLD.post_id;
    END IF;
    RETURN NULL;
